            ...
            <div class="add-role-container">
                ...
                try {
                    ...
                    if ($_SERVER["REQUEST_METHOD"] === "POST") {
                        $role_id = "R" . bin2hex(random_bytes(7.5));
                        $name = $_POST["name"];
                        $privileges = $_POST["privileges"]; // array of all privileges
                        //echo $role_id;
                        ...
                        foreach ($columns as $column) {
                            $request->bindValue(
                                ":" . strtolower($column),
                                isset($privileges[$column]) ? 1 : 0,
                            );
                        }

                        try {
                            $request->execute();
                            echo "<div class='success-message'>Role added successfully</div>";
                        } catch (PDOException $e) {
                            if ($e->getCode() == 23000) {
                                // Duplicate key error
                                echo "<div class='error'>Role ID already exists. Please try again.</div>";
                            } else {
                                echo "<div class='error'>Error: " .
                                    $e->getMessage() .
                                    "</div>";
                            }
                            // Optionally regenerate ID and retry
                        }
                    }

                    echo "
                            <form method='POST' action='add_roles.php' class='role-form'>
                        ...
                        foreach ($privileges as $priv) {
                            echo "
                                    <label class='privilege-card {$priv["action"]}'>
                                        <input type='checkbox' name='privileges[{$priv["column"]}]' value='1' id='{$priv["column"]}' class='privilege-checkbox'>
                                        <div class='privilege-content'>
                                            <span class='action-icon'>" .
                                getActionIcon($priv["action"]) .
                                "</span>
                ...
                function getCategoryIcon($category)
                {
                    switch ($category) {
                        case "Order States":
                            return '<ion-icon name="list-outline"></ion-icon>';
                        case "USERS":
                            return '<ion-icon name="people-outline"></ion-icon>';
                        case "ROLES":
                            return '<ion-icon name="key-outline"></ion-icon>';
                        case "PRODUCTS":
                            return '<ion-icon name="cube-outline"></ion-icon>';
                        case "ORDERS":
                            return '<ion-icon name="cart-outline"></ion-icon>';
                        case "INVOICES":
                            return '<ion-icon name="document-text-outline"></ion-icon>';
                        case "QUOTATIONS":
                            return '<ion-icon name="file-tray-outline"></ion-icon>';
                        case "STORAGE":
                            return '<ion-icon name="archive-outline"></ion-icon>';
                        case "SUPPLIERS":
                            return '<ion-icon name="business-outline"></ion-icon>';
                        case "AI":
                            return '<ion-icon name="hardware-chip-outline"></ion-icon>';
                        default:
                            return '<ion-icon name="ellipse-outline"></ion-icon>';
                    }
                }
                ...
                <div class="footer-links">
                    ...
                    <a href="show_roles.php">Back to roles list</a>
                </div>
            </div>
        </div>
    </div>
