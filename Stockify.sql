USE STOCKIFY_DATABASE;

DROP TRIGGER IF EXISTS update_tva_prices;
...
-- Create trigger to update the products prices each time the tva amount gets changed
DELIMITER $$

CREATE TRIGGER update_tva_prices
AFTER UPDATE ON GENERAL_VARIABLES
FOR EACH ROW
BEGIN
    IF NEW.ID = 1 THEN
        UPDATE PRODUCT
        SET price = price * (100 + NEW.TVA_AMOUNT) / (100 + OLD.TVA_AMOUNT);
    END IF;
END$$
